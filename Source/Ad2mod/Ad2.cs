...
public class Ad2
{
    ...
    {
        if (!forced && !Ad2Mod.settings.verboseLogging)
        {
            return;
        }
        ...
        if (warning)
        {
            Log.Warning($"[BulkRecipeGenerator]: {message}");
            return;
        }

        Log.Message($"[BulkRecipeGenerator]: {message}");
    }
            ...
            ((Action)(() =>
            {
                ...
                if (methodForPatch == null)
                {
                    LogMessage(
                        ...
                        " so RecipeIcons patch will not be applied.", false, true);
                    return;
                }
